...
The test suite is run via phpunit. Note that these are all live tests that must be run targeted at a real GridPane instance. Credentials can be provided by setting the environment variables in phpunit.xml; a sample is provided at phpunit.xml.dist.

To run the unit tests: `vendor/bin/phpunit --testsuite "GridPane API Unit Test Suites"`

To run the live tests: `vendor/bin/phpunit --testsuite "GridPane API Live Test Suites"`
 ...
 *
 * @var GridPane\API\Utilities\Auth
 */
```
...
When adding a resource, use traits to define available API calls. Resource traits are namespaced under `GridPane\API\Traits\Resource`.
...
Use `GridPane\API\Traits\Utility\InstantiatorTrait` when you want a resource to be chainable to other resources. See `GridPane/API/Resources/Servers.php`.
